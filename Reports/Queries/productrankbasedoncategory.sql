SELECT {topsub} ROW_NUMBER() OVER(ORDER BY(SELECT 1)) AS sr_No, 
...
slf.mcat, 
slf.scat 
FROM tbl_productrank WITH (nolock) LEFT OUTER JOIN 
v_barcode WITH (NOLOCK) ON v_barcode.inv_mast_uid = tbl_productrank.inv_mast_uid 
...
ORDER BY tbl_productrank.slqty DESC, tbl_productrank.item_id