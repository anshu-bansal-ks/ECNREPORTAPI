...
, inv_mast.item_id
, inv_mast.item_desc
, oe_line.source_loc_id as location_id
, oe_line.supplier_id
, dbo.p21_view_supplier.supplier_name
...
JOIN dbo.p21_view_supplier ON p21_view_supplier.supplier_id = oe_line.supplier_id
WHERE (@repId = 'ALL' OR oe_hdr_salesrep.salesrep_id = @repId )
AND (@supplierId = 'ALL' OR oe_line.supplier_id = TRY_CAST(@supplierId AS INT))
AND (@locationId = 'ALL' OR oe_line.source_loc_id = TRY_CAST(@locationId AS INT))
