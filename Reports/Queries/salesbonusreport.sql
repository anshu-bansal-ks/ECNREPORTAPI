...
, CASE WHEN ih.paid_in_full_flag = 'Y' THEN rd.payment_amount - freight ELSE rd.payment_amount END adjusted_amt
, CASE WHEN DATEDIFF(dd, invoice_date, rd.payment_date) <= 120 THEN (rd.payment_amount - freight) ELSE 0 END AgedIn
, CASE WHEN DATEDIFF(dd, invoice_date, rd.payment_date) > 120  THEN (rd.payment_amount - freight) ELSE 0 END AgedOut
